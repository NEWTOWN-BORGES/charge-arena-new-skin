...
#
# Uso: tools/export_apk.sh <saida.apk> [pacote] [nome da app] [versão]
...
python3 - "$work/export_presets.cfg" "$out" "$package" "$label" "$version" <<'EOF'
import re, sys
path, out, package, label, version = sys.argv[1:]
text = open(path).read()
